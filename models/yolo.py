...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for f, s in zip(m.f, m.stride):  #  from
            ...
            b_obj = b[:, 4] + math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            b_cls = b[:, 5:] + (math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum()))  # cls
            b_new = torch.cat([b[:, :4], b_obj.unsqueeze(1), b_cls], 1)
