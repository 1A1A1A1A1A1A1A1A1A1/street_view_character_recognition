...
本文件提供用于加载和预处理图像数据集的类和函数，主要功能包括：
1. 数据加载 - 支持从文件夹、文本文件列表、视频和网络摄像头加载数据
2. 数据预处理 - 图像缩放、填充、格式转换等基础处理
3. 数据增强 - 马赛克增强、随机仿射变换、HSV增强等高级增强方法
4. 批量处理 - 支持批量加载和预处理，兼容PyTorch DataLoader

主要类说明：
- LoadImages: 用于推理，从文件夹或文件加载图像/视频
- LoadWebcam: 用于推理，从网络摄像头实时加载图像
- LoadStreams: 用于推理，从多个IP摄像头或RTSP流加载图像
- LoadImagesAndLabels: 用于训练，加载图像和标签，支持数据增强

支持的数据格式：
- 图像: .bmp, .jpg, .jpeg, .png, .tif, .dng
- 视频: .mov, .avi, .mp4, .mpg, .mpeg, .m4v, .wmv, .mkv
- 标签: 期望格式为YOLO格式的.txt文件（每行：类别 x_center y_center width height，归一化坐标）

使用说明：
- 训练时使用create_dataloader函数创建数据加载器
- 推理时根据输入源类型选择相应的加载类(LoadImages/LoadWebcam/LoadStreams)
- 可通过设置参数控制数据增强强度和预处理方式
"""
