...
本文件包含YOLOv5模型所需的各种工具函数，主要功能包括：
1. 边界框处理与转换（xyxy/xywh格式转换、坐标缩放等）
2. IoU计算（多种IoU变体如GIoU、DIoU、CIoU）
3. 非极大值抑制(NMS)
4. 模型评估指标计算（AP、mAP、精确率、召回率等）
5. 数据集处理工具
6. 可视化工具（绘制边界框、损失曲线等）
7. 模型优化工具（锚点聚类、进化算法等）

使用说明：
- 导入方式: from utils.utils import *
- 大多数函数设计为与PyTorch模型兼容
- 可单独调用各函数或作为模型训练、评估过程的组件
"""
